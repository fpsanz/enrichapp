...
plotPCA(vst(dds), intgroup = params$variablepca, ntop = 300) +
            theme(plot.margin=unit(c(0.5,0.5,0.5,0.5),"cm")) +
            theme(text = element_text(size=12)) +
  #ggtitle("PCA: AAV variability in 'vsd' transformation") +
  scale_color_manual(values = c("#008000","#800080")) +
  ggrepel::geom_text_repel(aes(label = paste("",dds$name, sep = "")))
...
df = data.frame(AAV = dds_fc_median10$AAV)
...
pheatmap(mat, cellwidth= 10.5, cellheight = 8.5, cluster_rows=TRUE, show_colnames=TRUE, cluster_cols=TRUE, annotation_col = df, labels_col = paste( vsd$name, vsd$AAV, sep = " - " ), annotation_colors = ann_color)
